...
def call_filter(
    ...
    user_prompt = (
        ...
        "11) Score domain_breadth_score high only when the method is demonstrated or argued across broad tasks, datasets, domains, or a reusable abstract problem. "
        "domain_breadth_score >= 7 requires explicit cross-task, cross-domain, cross-dataset, or reusable abstract-problem evidence. "
        "Single-region, single-sea-area, single-species, single-animal/insect, single-device, small-cohort physiological, or single-task EEG applications should usually be <= 6.\n"
...
def _make_filter_client(model: str, max_output_tokens: int) -> LLMClient:
    client = make_task_client("filter", model=model or None)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(pending):
            ...
            try:
                ...
                if isinstance(exc, LLMAuthenticationError):
                    log("[ERROR] LLM 认证失败：请更新 DPR_LLM_API_KEY / DPR_LLM_BASE_URL 后重新运行。")
                    group_end()
                    raise
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Step 4: filter papers with an OpenAI-compatible LLM for recommendations.",
    )
    parser.add_argument(
        "--input",
        type=str,
    ...
    parser.add_argument(
        ...
        help="model for filter; defaults to DPR_LLM_FILTER_MODEL / DPR_LLM_MODEL.",
    )
    parser.add_argument(
        "--max-output-tokens",
        type=int,
