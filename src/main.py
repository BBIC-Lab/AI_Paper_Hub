...
def resolve_summary_step_env() -> dict[str, str]:
    env = os.environ.copy()
    summary_api_key = _read_env_text("DPR_LLM_API_KEY", "LLM_API_KEY", "OPENAI_API_KEY")
    summary_base_url = _read_env_text("DPR_LLM_BASE_URL", "LLM_BASE_URL", "OPENAI_BASE_URL")
    summary_model = _read_env_text("DPR_LLM_SUMMARY_MODEL", "DPR_LLM_MODEL", "LLM_MODEL")

    if summary_api_key:
        env["DPR_LLM_API_KEY"] = summary_api_key
    if summary_base_url:
        env["DPR_LLM_BASE_URL"] = summary_base_url
    if summary_model:
        env["DPR_LLM_SUMMARY_MODEL"] = summary_model
    return env
...
def main() -> None:
    ...
    if skip_rerank:
        print(
            f"[INFO] Step 3 - Rerank 已跳过：第一版不依赖专用 rerank API，"
            f"使用 RRF fallback。provider={rerank_base}",
            flush=True,
        )
