...
def _as_positive_int(v: Any, default: int) -> int:
  try:
    value = int(v)
  except Exception:
    return default
...
def _get_legacy_daily_paper_limit(profile: Dict[str, Any]) -> int:
  for key in ("daily_paper_limit", "daily_candidate_limit", "daily_limit"):
    if key in profile:
      return _as_positive_int(profile.get(key), DEFAULT_DAILY_SECTION_PAPER_LIMIT)
  return DEFAULT_DAILY_SECTION_PAPER_LIMIT


def _get_section_daily_paper_limit(profile: Dict[str, Any], section: str) -> int:
  section_key = f"{section}_daily_paper_limit"
  if section_key in profile:
    return _as_positive_int(profile.get(section_key), DEFAULT_DAILY_SECTION_PAPER_LIMIT)
  return _get_legacy_daily_paper_limit(profile)
...
def get_profile_daily_paper_limits(config: Dict[str, Any]) -> Dict[str, Dict[str, int]]:
  plan = build_pipeline_inputs(config or {})
  ...
  for profile in plan.get("profiles") or []:
    ...
    if not tag or tag.lower() not in active_tags:
      continue
    limits[tag] = {
      "deep": _as_positive_int(profile.get("deep_daily_paper_limit"), DEFAULT_DAILY_SECTION_PAPER_LIMIT),
