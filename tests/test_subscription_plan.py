...
class SubscriptionPlanTest(unittest.TestCase):
    ...
    def test_profile_daily_section_limits_default_explicit_and_legacy(self):
        cfg = {
            'subscriptions': {
                'intent_profiles': [
                    {
                    ...
                    },
                    {
                        'tag': 'CustomLimit',
                        'enabled': True,
                        'deep_daily_paper_limit': 6,
                        'quick_daily_paper_limit': 4,
                        'keywords': [{'keyword': 'y', 'query': 'y'}],
                    },
                    {
                        'tag': 'LegacyLimit',
                        'enabled': True,
                        'daily_paper_limit': 8,
                        'keywords': [{'keyword': 'z', 'query': 'z'}],
                    },
                ],
            }
        }
        plan = build_pipeline_inputs(cfg)
        limits = get_profile_daily_paper_limits(cfg)
        profile_limits = {
            item.get('tag'): {
                'deep': item.get('deep_daily_paper_limit'),
                'quick': item.get('quick_daily_paper_limit'),
                'legacy': item.get('daily_paper_limit'),
            }
            for item in plan['profiles']
        }

        self.assertEqual(profile_limits['DefaultLimit'], {'deep': 10, 'quick': 10, 'legacy': None})
        self.assertEqual(profile_limits['CustomLimit'], {'deep': 6, 'quick': 4, 'legacy': None})
        self.assertEqual(profile_limits['LegacyLimit'], {'deep': 8, 'quick': 8, 'legacy': None})
        self.assertEqual(
            limits,
            {
                'DefaultLimit': {'deep': 10, 'quick': 10},
                'CustomLimit': {'deep': 6, 'quick': 4},
