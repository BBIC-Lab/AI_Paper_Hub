...
class PeriodicReportsTest(unittest.TestCase):
    ...
    def test_metrics_and_report_outputs_include_charts_and_sidebar(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("dpr-periodic-weekly-v4", readme)
            self.assertIn("关注主题", readme)
            self.assertIn("其他主题", readme)
    ...
    def test_weekly_v2_chart_markup_uses_cloud_bundle_and_context_heatmap(self):
        ...
        self.assertEqual(many_heat_html.count("dpr-weekly-heat-row"), 12)
        self.assertIn("topic-11", many_heat_html)
        self.assertNotIn("topic-12", many_heat_html)
