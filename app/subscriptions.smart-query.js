...
window.SubscriptionsSmartQuery = (function () {
  ...
  const applyAddModal = () => {
    ...
    const nextDailyPaperLimits = {
      deep: normalizeDailyPaperLimit(
        document.getElementById('dpr-add-deep-daily-paper-limit')?.value || modalState.deep_daily_paper_limit,
      ),
      quick: normalizeDailyPaperLimit(
  ...
  const applyChatSelection = () => {
    ...
    const dailyPaperLimits = {
      deep: normalizeDailyPaperLimit(
        document.getElementById('dpr-chat-deep-daily-paper-limit')?.value || modalState.deep_daily_paper_limit,
      ),
      quick: normalizeDailyPaperLimit(
  ...
  const handleModalInput = (e) => {
    ...
    if (!target || !target.matches) return;
    if (
      target.id === 'dpr-chat-deep-daily-paper-limit' ||
      target.id === 'dpr-add-deep-daily-paper-limit'
    ) {
      if (modalState) {
        modalState.deep_daily_paper_limit = normalizeDailyPaperLimit(target.value);
      }
      return;
    }
    if (
      target.id === 'dpr-chat-quick-daily-paper-limit' ||
      target.id === 'dpr-add-quick-daily-paper-limit'
    ) {
      if (modalState) {
